...
router.post('/send-email', async (req, res) => {
  ...
  //}
  const getDatos = () => {
  // promise es una funcion con 2 argumento
  // resolve devuelve lo resuelto
  // reject devuelve por erro
...
*/
  ...
  */
  ...
  // consulto si existe el cliente... por el numero de celular
  ...
  var clienteNuevo = ""
  console.log("paso lileasql ", linealsql)
  const pro = await pool.query(linealsql, function (err, result) {
    ...
    if (result.length > 0) {
      ...
      //linea = "UPDATE cliente set 'acum_venta' = " + (msal + totalgralstr2) + " WHERE telefono = '" + cel + "'"
      newsaldo = msal + totalgralstr2;
      Actuali_cliente(newsaldo, mid)
      ...
      //Graba_pedido(mid, totalgralstr2, fecha, horalocal, nota)
    ...
    if (clienteNuevo === "true") {
      ...
      var lineadesql = "INSERT INTO cliente values (NULL,'" + cliente + "', '" + direccion + "', '" + localidad + "', '" + cel + "', '" + " " + "', '" + " " + "', '" + correo + "', '" + 1 + "', '" + 0 + "', '" + totalgralstr2 + "')";
      cliente_cod =  nuevocliente(lineadesql)
      console.log("cliente nuevo despues de la funcion = ", cliente_cod)
      cliente_sal="0"
     // Graba_pedido(cliente_cod, totalgralstr2, fecha, horalocal, nota)
      lineasql = "INSERT INTO pedido values (NULL,'" + "llega" + "', '" + "0" + "', '" + "particular" + "', '" +
...
async function nuevocliente(lineadesql) {
  console.log("linea de cliente en nuevo cliente", lineadesql);
  const result = await pool.query(lineadesql);
  console.log("Cliente codigo de la funcion nuevocliente ", result.insertId);
  cliente_des = result.nombre
  cliente_tel = result.telefono
  cliente_dir = result.direccion
  cliente_cod = result.id
  cliente_sal = result.saldo
}




async function Actuali_cliente(total, celular) {
  total = total + 1;
  console.log("total funcion", total);
  console.log("celular funcion", celular);
  lineasql = "UPDATE cliente set acum_venta = " + total + " WHERE id = " + mid;
  console.log("linea sql : ", lineasql);
  const cli = await pool.query(lineasql, function (err, result) {
    if (err) throw err;
...
router.post('/graba_cliente', async (req, res) => {
  ...
  const newProducto = req.body;
  const result = await pool.query('INSERT INTO producto SET ? ', newProducto);
  console.dir('resul', result.insertId);
  // console.log('errores',err);
  if (result.insertId < 1) {
    console.log('error :');
    Swal.fire({
